...
2. **Look at existing schema first.** Don't propose tables that already exist. Don't propose patterns that contradict the rest.
3. **Confirm the question before writing SQL.** "Add a column" vs "redesign normalization" need different answers.
4. **Test queries against actual data** when possible — `psql`, `sqlite3`, or the project's CLI.
