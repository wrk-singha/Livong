...
// DevLogin bypasses OTP for local testing of admin tooling. ONLY register this
// route when LIVONG_DEV_LOGIN=1 is set — see main.go. Mirrors backend's pattern.
...
func GenerateToken(userID string) (string, error) {
	...
	claims := Claims{
		...
		RegisteredClaims: jwt.RegisteredClaims{
			...
			Issuer:    "livong-admin",
			Audience:  jwt.ClaimStrings{"livong-admin-api"},
		},
	}
...
func ValidateToken(tokenStr string) (*Claims, error) {
	...
	}, jwt.WithIssuer("livong-admin"), jwt.WithAudience("livong-admin-api"))
	if err != nil {
		return nil, err
	}
